...
function hiddenStateOf(entry) {
  ...
  var value = settings.hidden
  if (value === true || value === "hover") return "hover"
  if (value === "always") return "always"
  return "shown"
}

// Bartender grouping: hidden widgets render beside the tray so the reveal
// expands out of the tray itself. Purely a rendering order — the config
// keeps the user's real layout. Sections without a tray keep hidden entries
// where they are. The center anchor entry is never hoisted.
function groupHiddenAtTray(entries, section, anchorId) {
  var values = Array.isArray(entries) ? entries : []
  var trayIndex = -1
  for (var i = 0; i < values.length; i++) {
    if (/\.tray$/.test(entryId(values[i]))) { trayIndex = i; break }
  }
  if (trayIndex === -1) return values

  var anchor = String(anchorId || "")
  var tray = values[trayIndex]
  var hidden = []
  var rest = []
  for (var j = 0; j < values.length; j++) {
    if (j === trayIndex) continue
    var entry = values[j]
    if (hiddenStateOf(entry) !== "shown" && entryId(entry) !== anchor) hidden.push(entry)
    else rest.push(entry)
  }
  if (section === "right") return [tray].concat(hidden, rest)
  return rest.concat(hidden, [tray])
}
...
function pinTrayToInner(entries, section) {
  ...
  var values = Array.isArray(entries) ? entries : []
  for (var i = 0; i < values.length; i++) {
    if (/\.tray$/.test(entryId(values[i]))) trayEntry = values[i]
