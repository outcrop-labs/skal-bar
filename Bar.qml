...
Item {
  ...
  // optional (not `required`) because the host loads plugin bars through a
  // URL-based Loader, which cannot initialize required properties — the host
  // fills these via configureBar() immediately after load.
  ...
  // Bumped by the inline-settings patch path, which mutates layoutConfig
  // arrays in place. Bindings derived from layout entries (e.g. the hidden
  // widget chevrons) read this to re-evaluate without a full rebuild.
  property int settingsSerial: 0
  ...
  // ---- skal.bar customization -------------------------------------------
  // Noctalia-style appearance options, all optional. Defaults reproduce the
  // stock omarchy bar: theme size, docked full-bleed, theme background,
  // no gap between widgets. Set them on the `bar` subtree of shell.json.
  property int customHeight: 0        // >0 = bar thickness (px, font-scaled); 0 = theme size
  property int barMargin: 0           // >0 = floating bar, offset this far from screen edges
  property int barRadius: 0           // corner radius (clamped to half the bar size)
  property string backgroundHex: ""   // "" = theme background; else #rrggbb override
  property real backgroundOpacity: 1  // 0..1 alpha for the bar background
  property int widgetSpacing: 0       // gap between widgets inside a section
  property int edgePadding: -1        // -1 = theme default; else padding at section outer edges
  // Bartender-style hiding: layout entries with "hidden": true collapse to
  // zero width until revealed. "hover" reveals while the pointer is on the
  // bar; "click" only via the chevron toggle. Clicking the chevron in either
  // mode pins the reveal open until clicked again.
  property string hiddenReveal: "hover"
  // Per-region overrides for the reveal gesture and the indicator glyph,
  // set from the settings panel: hiddenRevealByRegion / revealIcons maps
  // keyed by section. Regions without an entry fall back to the global
  // hiddenReveal and the default "›" glyph (the tray section keeps its own
  // chevron icon unless one is configured).
  property var hiddenRevealByRegion: ({})
  property var revealIcons: ({})
  // Per-region exposure control: each section's hidden widgets open and
  // close through their own icon — the tray cluster chevron for the tray's
  // section, a dedicated chevron for the others. Pinned is the click state;
  // the hover latch holds a region open after its expansion moves the icon
  // out from under the pointer (a bare hover flag would oscillate), until
  // the pointer leaves the bar.
  property var hiddenPinnedByRegion: ({ left: false, center: false, right: false })
  property var regionLatch: ({})
  property bool trayHovered: false

  onTrayHoveredChanged: if (trayHovered && regionRevealMode(traySection) !== "click") setRegionLatch(traySection, true)
  onBarHoveredChanged: if (!barHovered) regionLatch = ({})
  // Native tray chrome (TrayCluster.qml): which section's inner edge hosts
  // the tray, and the persisted pinned/hidden tray item ids. "none" hides
  // the tray entirely.
  property string traySection: "right"
  property var trayConfig: null
  readonly property bool trayEnabled: traySection === "left" || traySection === "right"

  function setRegionLatch(region, on) {
    var key = String(region || "")
    if (key !== "left" && key !== "center" && key !== "right") return
    var next = {}
    for (var k in regionLatch) next[k] = regionLatch[k]
    if (on) next[key] = true
    else delete next[key]
    regionLatch = next
  }

  function toggleRegionPinned(region) {
    var key = String(region || "")
    if (key !== "left" && key !== "center" && key !== "right") return
    var next = {}
    for (var k in hiddenPinnedByRegion) next[k] = hiddenPinnedByRegion[k]
    next[key] = !hiddenPinnedByRegion[key]
    hiddenPinnedByRegion = next
  }

  // The tray section's unified exposure: drawer + that section's hidden
  // widgets, driven by the tray cluster chevron.
  function trayExposureOpen() {
    return hiddenPinnedByRegion[traySection] === true || regionLatch[traySection] === true
  }

  function regionRevealMode(region) {
    var key = String(region || "")
    var value = String(hiddenRevealByRegion[key] || "").toLowerCase()
  ...
  function regionRevealed(region) {
    var key = String(region || "")
    if (!sectionHasHidden(key)) return false
    if (hiddenPinnedByRegion[key] === true) return true
    if (regionLatch[key] === true) return true
    return regionRevealMode(key) !== "click" && barHovered
  }
  readonly property bool floating: barMargin > 0
  readonly property int sectionEdgePadding: edgePadding >= 0 ? edgePadding : Style.space(8)
  // A shaped background (radius, translucency, or color override) must be
  // painted inside the window; a flat docked bar can keep painting the
  // window surface itself.
  readonly property bool shapedBackground: !transparent && (barRadius > 0 || backgroundOpacity < 1 || backgroundHex !== "")
  ...
  function entryHiddenState(entry) {
    var value = entrySettings(entry).hidden
  ...
  function entryHidden(entry) {
    return entryHiddenState(entry) !== "shown"
  }

  // Revealable hidden widgets only ("hover" state). "Always hidden" entries
  // never join the reveal, so a section holding only those shows no chevron.
  function sectionHasHidden(region) {
    var serial = settingsSerial
    var entries = layoutEntries(region)
    for (var i = 0; i < entries.length; i++) {
      if (entryHiddenState(entries[i]) === "hover") return true
    }
    return false
  }

  // Section entry partitions: shown entries render through the normal
  // module lists; hover-hidden entries render inside that section's
  // HiddenDrawer (the sliding reveal); always-hidden entries render nowhere.
  function shownEntries(region) {
    var entries = layoutEntries(region)
    var out = []
    for (var i = 0; i < entries.length; i++) {
      if (entryHiddenState(entries[i]) === "shown") out.push(entries[i])
    }
    return out
  }

  function revealEntries(region) {
    var entries = layoutEntries(region)
    var out = []
    for (var i = 0; i < entries.length; i++) {
      if (entryHiddenState(entries[i]) === "hover") out.push(entries[i])
    }
    return out
  }

  // The native tray cluster owns the reveal affordance (its chevron pins).
  // Without a tray, each section falls back to its own chevron.

  readonly property int barSize: {
    var themed = vertical ? Style.bar.sizeVertical : Style.bar.sizeHorizontal
    if (customHeight <= 0) return themed
    // Mirror Style.barToken: custom heights scale with the bar font scale so
    // a taller bar set at 12px grows with the system font like theme sizes.
    var scale = Style.barScaleWithFont ? Style.fontScale : 1
    return Math.max(1, Math.round(customHeight * scale))
  }
  ...
  // The tray is native chrome (TrayCluster), never a layout entry: strip
  // any leftover *.tray entries defensively. Hidden widgets hoist to their
  // section's inner edge so the reveal expands out of the tray cluster —
  // Bartender-style. The center anchor entry stays where the user put it.
  function normalizeLayout(layout) {
    var normalized = Util.normalizeLayout(Util.isPlainObject(layout) ? layout : fallbackBarConfig.layout)
    return {
      left:   hoistHidden(stripTrayEntries(normalized.left), false),
      center: hoistHidden(stripTrayEntries(normalized.center), false),
      right:  hoistHidden(stripTrayEntries(normalized.right), true)
    }
  }

  function stripTrayEntries(entries) {
    var values = Array.isArray(entries) ? entries : []
    var out = []
    for (var i = 0; i < values.length; i++) {
      if (!/\.tray$/.test(entryId(values[i]))) out.push(values[i])
    }
    return out
  }

  function hoistHidden(entries, atStart) {
    var values = Array.isArray(entries) ? entries : []
    var hidden = []
    var rest = []
    for (var i = 0; i < values.length; i++) {
      ...
      if (entryHiddenState(entry) !== "shown" && id !== centerAnchor) hidden.push(entry)
      else rest.push(entry)
    }
    return atStart ? hidden.concat(rest) : rest.concat(hidden)
  }
  ...
  function groupHiddenAtTray(entries, section, anchorId) {
    return BarModel.groupHiddenAtTray(entries, section, anchorId)
  }
  ...
  function applyBarConfig() {
    ...
    customHeight = clampInt(config.height, 0, 200)
    barMargin = clampInt(config.margin, 0, 128)
    barRadius = clampInt(config.radius, 0, 64)
    widgetSpacing = clampInt(config.widgetSpacing, 0, 64)
    edgePadding = clampInt(config.edgePadding, -1, 128)
    backgroundOpacity = Math.max(0, Math.min(1, Number(config.backgroundOpacity) || 1))
    backgroundHex = /^#[0-9A-Fa-f]{3,8}$/.test(String(config.backgroundColor || "")) ? String(config.backgroundColor) : ""
    var reveal = String(config.hiddenReveal || "").trim().toLowerCase()
    hiddenReveal = reveal === "click" ? "click" : "hover"
    var section = String(config.traySection || "").trim().toLowerCase()
    traySection = (section === "left" || section === "right" || section === "none") ? section : "right"
    trayConfig = Util.isPlainObject(config.tray) ? config.tray : ({})
    hiddenRevealByRegion = Util.isPlainObject(config.hiddenRevealByRegion) ? config.hiddenRevealByRegion : ({})
    revealIcons = Util.isPlainObject(config.revealIcons) ? config.revealIcons : ({})

    if (!trayEnabled) hiddenPinned = false
    ...
    // A hidden-state change is the exception: hidden widgets render beside
    // the tray, so the visual order changes and a full rebuild is needed.
    var next = normalizeLayout(config.layout)
    var delta = BarModel.inlineSettingsDelta(layoutConfig, next)
    if (delta && !deltaChangesHiddenState(delta)) {
  ...
  function deltaChangesHiddenState(delta) {
    for (var i = 0; i < delta.length; i++) {
      var change = delta[i]
      var current = layoutConfig[change.region][change.index]
      if (BarModel.hiddenStateOf(current) !== BarModel.hiddenStateOf(change.entry)) return true
    }
    return false
  }
  ...
  function applySettingsDelta(delta) {
    for (var i = 0; i < delta.length; i++) {
      ...
      for (var s = 0; s < moduleSlots.length; s++) {
        ...
        // Re-point the slot's entry so bar-level entry flags (e.g. the
        // Bartender "hidden" toggle) re-evaluate; the Repeater never sees
        // in-place array element swaps.
  ...
  component BarPanel: PanelWindow {
    ...
    // Auto exclusion keeps window tops tracking the bar's real geometry:
    // the reserved zone includes the bar's layer margins, so a floating
    // (offset) or taller/shorter bar moves window tops accordingly, with
    // Hyprland's gaps_out landing between the bar and the windows.
    ...
    margins {
      top: root.barHidden && root.position === "top" ? -root.barSize : (root.position === "top" || root.vertical ? root.barMargin : 0)
      bottom: root.barHidden && root.position === "bottom" ? -root.barSize : (root.position === "bottom" || root.vertical ? root.barMargin : 0)
      left: root.barHidden && root.position === "left" ? -root.barSize : (root.position === "left" || !root.vertical ? root.barMargin : 0)
      right: root.barHidden && root.position === "right" ? -root.barSize : (root.position === "right" || !root.vertical ? root.barMargin : 0)
    }
    ...
    color: root.transparent || root.shapedBackground ? "transparent" : root.background
    surfaceFormat.opaque: false
    WlrLayershell.namespace: "omarchy-bar"
    WlrLayershell.layer: WlrLayer.Top

    // Shaped backgrounds (radius / translucency / color override) paint here
    // so the window surface itself can stay transparent for real corners.
    Rectangle {
      anchors.fill: parent
      visible: root.shapedBackground
      color: root.backgroundHex !== "" ? root.backgroundHex : root.background
      opacity: root.backgroundOpacity
      radius: Math.min(root.barRadius, Math.min(width, height) / 2)
    ...
    Component {
      ...
      Item {
        ...
        Row {
          ...
          anchors.leftMargin: root.sectionEdgePadding
          ...
          TrayCluster {
            bar: root
            visible: root.traySection === "left"
          }
        }

        Row {
          anchors.right: parent.right
          anchors.rightMargin: root.sectionEdgePadding
          anchors.verticalCenter: parent.verticalCenter
          spacing: root.widgetSpacing

          TrayCluster {
            bar: root
            visible: root.traySection === "right"
          }
    ...
    Component {
      ...
      Item {
        ...
        Column {
          ...
          anchors.topMargin: root.sectionEdgePadding
        ...
        Column {
          ...
          anchors.bottomMargin: root.sectionEdgePadding
          anchors.horizontalCenter: parent.horizontalCenter
          spacing: root.widgetSpacing
  ...
  component LeftModules: ModuleList {
    entries: root.shownEntries("left")
    region: "left"
  }

  component RightModules: ModuleList {
    entries: root.shownEntries("right")
    region: "right"
  }
  ...
  component CenterModules: Item {
    ...
    // Hidden entries never render inline here — the section's HiddenDrawer
    // (placed after the group) owns them.
    readonly property var entries: root.shownEntries("center")
  ...
  // The section's sliding reveal: a clipping container whose width animates
  // between zero and its content, with the content anchored to the inner
  // edge — the same reveal mechanic as the tray drawer. Hover-hidden
  // widgets render inside; always-hidden ones render nowhere.
  component HiddenDrawer: Item {
    id: drawer

    property string region: ""
    readonly property var drawerEntries: root.revealEntries(region)
    readonly property bool open: drawerEntries.length > 0 && root.regionRevealed(region)
    property real progress: open ? 1 : 0

    // Direction depends on the section: right-section drawers grow leftward
    // (content pinned right), left/center drawers grow rightward.
    readonly property bool growLeft: region === "right"
    readonly property real contentBreadth: root.vertical
    ...
    clip: true
    visible: drawerEntries.length > 0
    width: root.vertical ? (root.barSize) : contentBreadth * progress
    height: root.vertical ? contentBreadth * progress : root.barSize

    ModuleList {
      id: content
      entries: drawer.drawerEntries
      region: drawer.region
      revealDrawer: true
      anchors.right: !root.vertical && drawer.growLeft ? parent.right : undefined
      anchors.left: !root.vertical && !drawer.growLeft ? parent.left : undefined
      anchors.verticalCenter: !root.vertical ? parent.verticalCenter : undefined
      anchors.bottom: root.vertical && drawer.growLeft ? parent.bottom : undefined
      anchors.top: root.vertical && !drawer.growLeft ? parent.top : undefined
      anchors.horizontalCenter: root.vertical ? parent.horizontalCenter : undefined
    }
  }

  // A section's chevron adjacent to its drawer. Suppressed in the tray's
  // section, where the tray chevron owns the toggle.
  component SectionToggle: Item {
    id: toggleRoot

    property string region: ""
    readonly property bool active: root.sectionHasHidden(region)
      && !(root.trayEnabled && region === root.traySection)

    implicitWidth: active ? sectionChevron.implicitWidth : 0
    implicitHeight: active ? sectionChevron.implicitHeight : 0
    width: implicitWidth
    ...
    SectionChevron {
      id: sectionChevron
      anchors.fill: parent
      region: toggleRoot.region
      active: toggleRoot.active
    }
  }
  ...
  component ModuleList: Loader {
    ...
    property string region: ""
    // Drawers host always-expanded slots; the container animates exposure.
    property bool revealDrawer: false
    readonly property bool hasHidden: root.sectionHasHidden(region)
      ...
      Row {
        ...
        Repeater {
          ...
          ModuleSlot {
            ...
            slotInDrawer: moduleListRoot.revealDrawer
          }
        }
  ...
  // Per-region reveal toggle rendered at the inner edge of each section
  // that holds hidden widgets — suppressed in the tray's section, where the
  // tray cluster chevron owns the exposure. Click pins/unpins this region's
  // reveal; hovering latches it open until the pointer leaves the bar.
  component SectionChevron: Item {
    id: chevronRoot

    property string region: ""
    property bool active: false

    readonly property bool open: root.regionRevealed(region)
    readonly property string icon: root.revealIconFor(region)
    // Chevrons and arrows swing open; glyph icons (dots, bars) crossfade
    // into an X instead.
    readonly property bool swings: icon !== "⋯" && icon !== "≡"

    implicitWidth: active ? Math.max(iconBtn.implicitWidth, xBtn.implicitWidth) : 0
    implicitHeight: active ? Math.max(iconBtn.implicitHeight, xBtn.implicitHeight) : 0
    width: implicitWidth
    height: implicitHeight
    visible: active

    HoverHandler {
      onHoveredChanged: if (hovered && root.regionRevealMode(chevronRoot.region) !== "click") root.setRegionLatch(chevronRoot.region, true)
    }

    property real swing: open && swings ? (root.vertical ? 90 : 180) : 0
    Behavior on swing {
      NumberAnimation { duration: 350; easing.type: Easing.OutCubic }
    }
    ...
    BarIconButton {
      ...
      bar: root
      anchors.fill: parent
      text: chevronRoot.icon
      textRotation: (root.vertical ? 90 : 0) + chevronRoot.swing
      opacity: chevronRoot.open ? (chevronRoot.swings ? 0.6 : 0) : 1
      enabled: !(chevronRoot.open && !chevronRoot.swings)
      tooltipText: chevronRoot.open ? "Collapse hidden widgets" : "Reveal hidden widgets"
      ...
      onPressed: function(button) {
        if (button === Qt.LeftButton) root.toggleRegionPinned(chevronRoot.region)
      }
    }
    ...
    BarIconButton {
      ...
      anchors.fill: parent
      text: "✕"
      opacity: chevronRoot.open && !chevronRoot.swings ? 1 : 0
      enabled: chevronRoot.open && !chevronRoot.swings
      tooltipText: "Collapse hidden widgets"
  ...
  component ModuleSlot: Item {
    ...
    // collapse to zero size until revealed (hover or chevron pin).
    // "always" stays collapsed even while the rest are revealed.
    readonly property string hiddenState: root.entryHiddenState(entry)
    readonly property bool collapsed: !slotInDrawer && hiddenState !== "shown"
      && (hiddenState === "always" || !root.regionRevealed(slot.region))
    // Assigned by the enclosing ModuleList's Repeater context below.
    property bool slotInDrawer: false
    ...
    visible: !collapsed
    implicitWidth: !collapsed && activeItem && activeItem.visible ? (root.vertical ? root.barSize : activeItem.implicitWidth) : 0
    implicitHeight: !collapsed && activeItem && activeItem.visible ? activeItem.implicitHeight : 0
    width: implicitWidth
    height: implicitHeight
