...
// System tray as native skal.bar chrome: instantiated directly by Bar.qml at
// the tray section's inner edge — not a registry bar-widget. The chevron's
// left click toggles the bar's Bartender hidden-widget reveal; hover opens
// the tray drawer. Tray state (pinned/hidden items) persists under the
// bar.tray key of shell.json.
Item {
  id: root

  property var bar: null
  readonly property var traySettings: bar && bar.trayConfig ? bar.trayConfig : ({})

  Component.onDestruction: if (bar) bar.trayHovered = false
  readonly property bool vertical: bar ? bar.vertical : false
  readonly property int barSize: bar ? bar.barSize : Style.bar.sizeHorizontal

  // Unified with the bar's Bartender reveal: the drawer opens while the
  // tray is hovered (latched by the bar) or the reveal is pinned.
  readonly property string revealGlyph: {
    if (bar && bar.revealIcons && bar.traySection && bar.revealIcons[bar.traySection]) {
      return String(bar.revealIcons[bar.traySection])
    }
    return "\u203a"
  }
  ...
  readonly property var pinnedIds: traySettings.pinned instanceof Array ? traySettings.pinned : []
  readonly property var hiddenIds: traySettings.hidden instanceof Array ? traySettings.hidden : []
  ...
  // the cluster to zero width — icons paint outside a dead hit area.
  width: implicitWidth
    ...
    Item {
      ...
      Item {
        ...
        HoverHandler {
          onHoveredChanged: {
            if (!root.bar) return
            root.bar.trayHovered = hovered
            if (hovered && typeof root.bar.regionRevealMode === "function"
                && root.bar.regionRevealMode(root.bar.traySection) !== "click") {
              root.bar.setRegionLatch(root.bar.traySection, true)
            }
          }
        }
        ...
        Item {
          ...
          readonly property bool open: root.expanded
          readonly property string icon: root.revealGlyph
          readonly property bool swings: icon !== "⋯" && icon !== "≡"
          property real swing: open && swings ? 180 : 0
          ...
          implicitWidth: Math.max(trayIconBtn.implicitWidth, trayXBtn.implicitWidth)
          implicitHeight: Math.max(trayIconBtn.implicitHeight, trayXBtn.implicitHeight)
          width: implicitWidth
          height: implicitHeight
          ...
          BarIconButton {
            ...
            textRotation: expandIcon.swing
            opacity: expandIcon.open ? (expandIcon.swings ? 0.6 : 0) : 1
            enabled: !(expandIcon.open && !expandIcon.swings)
            ...
            onPressed: function(button) {
              ...
              else if (button === Qt.LeftButton && root.bar && typeof root.bar.toggleRegionPinned === "function") root.bar.toggleRegionPinned(root.bar.traySection)
            }
          }
          ...
          BarIconButton {
            ...
            anchors.fill: parent
            text: "✕"
            opacity: expandIcon.open && !expandIcon.swings ? 1 : 0
            enabled: expandIcon.open && !expandIcon.swings
    ...
    Item {
      ...
      Item {
        ...
        Item {
          ...
          readonly property string icon: root.revealGlyph
          readonly property bool swings: icon !== "⋯" && icon !== "≡"
          property real swing: open && swings ? 90 : 0
          ...
          BarIconButton {
            ...
            anchors.fill: parent
            text: expandIcon.icon
            textRotation: 90 + expandIcon.swing
            opacity: expandIcon.open ? (expandIcon.swings ? 0.6 : 0) : 1
            enabled: !(expandIcon.open && !expandIcon.swings)
